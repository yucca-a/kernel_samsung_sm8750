...
{
	struct wb_switch_state *state = container_of(work, struct wb_switch_state,
						    work);
	struct bdi_writeback *new_wb = state->wb;
	struct inode_switch_wbs_context *isw, *next_isw;
	struct llist_node *list;

	list = llist_del_all(&new_wb->switch_state->contexts);
	/*
...
static void wb_queue_isw(struct bdi_writeback *wb,
			 struct inode_switch_wbs_context *isw)
{
	if (llist_add(&isw->list, &wb->switch_state->contexts))
		queue_work(isw_wq, &wb->switch_state->work);
}
