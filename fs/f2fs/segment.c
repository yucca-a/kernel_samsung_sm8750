			...
			unsigned int *newseg, bool new_sec, bool pinning)
{
	...
	if (sec_alloc_pin) {
		...
		if (hint >= total_secs)
			hint = 0;
	} else if (sbi->sec_reserved_blocks) {
		total_secs = MAIN_SECS(sbi) - div_u64(sbi->sec_reserved_blocks + BLKS_PER_SEC(sbi) - 1,
						BLKS_PER_SEC(sbi));
		total_zones = total_secs / sbi->secs_per_zone;
		need_restore = true;
		if (hint >= total_secs)
...
find_other_zone:
	...
	need_from_first = secno >= total_secs;
	if (need_restore) {
		total_secs = MAIN_SECS(sbi);
		total_zones = total_secs / sbi->secs_per_zone;
		need_restore = false;
