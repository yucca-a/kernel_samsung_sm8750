...
static int restricted_cdev_count;
...
static bool is_valid_cdev(int i)
{
	return (IS_ERR_OR_NULL(prev_cdevs[i].cdev)) ? false : true;
}

static void release_prev_cdev(struct thermal_cooling_device *cdev)
{
	int i;
	...
	for (i = 0; i < restricted_cdev_count; i++) {
		...
		if (strcmp(cdev->type, prev_cdevs[i].cdev->type))
			continue;

		if (prev_cdevs[i].is_start == false)
			continue;

		/* Release only when cdev is restricted */
		prev_cdevs[i].last_exited_at = ktime_to_ms(ktime_get());
		prev_cdevs[i].accumulated +=
				(prev_cdevs[i].last_exited_at - prev_cdevs[i].last_entered_at);

		prev_cdevs[i].is_start = false;
		prev_cdevs[i].is_end = true;

		pr_debug("%s: cdev type(%s), target(%ld), accumulated(%lld.%03llds)\n",
...
void restrict_prev_cdev(struct thermal_cooling_device *cdev, unsigned long target)
{
	int i;
	bool isRestrictedOnce = false;
	...
	for (i = 0; i < restricted_cdev_count; i++) {
		...
		/* When a specific cdev has more than one restriction */
		if (prev_cdevs[i].is_start == false) {
			prev_cdevs[i].is_start = true;
			prev_cdevs[i].last_entered_at = ktime_to_ms(ktime_get());
			prev_cdevs[i].last_exited_at = 0;
		}

		if (prev_cdevs[i].target < target)
			prev_cdevs[i].target = target;

		isRestrictedOnce = true;
		prev_cdevs[i].is_end = false;
	...
	/* First restriction when the list is empty or new restricted cdev is added */
	if (!isRestrictedOnce) {
		prev_cdevs[i].cdev = cdev;
		prev_cdevs[i].target = target;
		prev_cdevs[i].accumulated = 0;
		prev_cdevs[i].is_start = true;
		prev_cdevs[i].is_end = false;
		prev_cdevs[i].last_entered_at = ktime_to_ms(ktime_get());
		prev_cdevs[i].last_exited_at = 0;

		restricted_cdev_count++;
...
void update_restricted_cdev(struct thermal_cooling_device *cdev, unsigned long target)
{
	if (target == 0)
		release_prev_cdev(cdev);
	else if (target > 0)
		restrict_prev_cdev(cdev, target);
}
EXPORT_SYMBOL_GPL(update_restricted_cdev);
...
static char prev_cdevs_buf[MAX_BUF_LEN];
static char *prev_cdevs_buf_ptr;
...
static void prev_cdev_print(void)
{
	...
	for (i = 0; i < restricted_cdev_count; i++) {
		...
		/* Initialize after checking all prev_cdevs */
		prev_cdevs[i].cdev = NULL;
		prev_cdevs[i].target = 0;
		prev_cdevs[i].accumulated = 0;
		prev_cdevs[i].is_start = false;
		prev_cdevs[i].is_end = false;
		prev_cdevs[i].last_entered_at = 0;
		prev_cdevs[i].last_exited_at = 0;
	}

	restricted_cdev_count = 0;
